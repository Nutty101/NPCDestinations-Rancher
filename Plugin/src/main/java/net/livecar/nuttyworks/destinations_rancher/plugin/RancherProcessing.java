...
import net.livecar.nuttyworks.destinations_rancher.DestinationsRancher;
...
public class RancherProcessing {

    public void pluginTick() {

        if (DestinationsRancher.Instance.breedingHistory != null) {
            Iterator<Entry<UUID, Date>> breedingIterator = DestinationsRancher.Instance.breedingHistory.entrySet().iterator();
        ...
        if (DestinationsRancher.Instance.milkingHistory != null) {
            Iterator<Entry<UUID, Date>> milkingIterator = DestinationsRancher.Instance.milkingHistory.entrySet().iterator();
        ...
        if (DestinationsRancher.Instance.monitoredNPCs == null)
            return;

        for (Entry<Integer, LocationSetting> entry : DestinationsRancher.Instance.monitoredNPCs.entrySet()) {
            ...
            NPCSetting ranchSet = DestinationsRancher.Instance.npcSettings.get(entry.getKey());
            ...
            if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.FINE, "Rancher_Processing.pluginTick|NPC:" + npc.getId() + "|Current Action|" + ranchSet.currentAction.toString());
            ...
            case ABORTING:
                if (ranchSet.destinationsTrait != null && ranchSet.destinationsTrait.currentLocation != null && ranchSet.destinationsTrait.currentLocation.destination.distanceSquared(npc.getEntity().getLocation()) > 3) {
                    if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.FINEST, "Rancher_Processing.pluginTick|NPC:" + npc.getId() + "|Not at destination|"
                    ...
                    DestinationsPlugin.Instance.getPathClass.addToQueue(npc, ranchSet.destinationsTrait, npc.getEntity().getLocation().add(0.0D, -1.0D, 0.0D), ranchSet.destinationsTrait.currentLocation.destination, 120,
                            new ArrayList<Material>(), 0, true, true, true, "DestinationsRancher");
                } else {
                    /*
            ...
            case TRAVERSING:
                if (ranchSet.currentDestination != null && !npc.getNavigator().isNavigating() && ranchSet.currentDestination.distanceSquared(npc.getEntity().getLocation()) < 4) {
                    ...
                    ranchSet.lastAction = new Date();
                    if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.FINEST, "Rancher_Processing.pluginTick|NPC:" + npc.getId() + "|Activity Timeout");
                }
                break;
            case IDLE:
            default:
                ...
                if (!rancherLocation.regionName.equals("")) {
                    ...
                    if (!DestinationsRancher.Instance.getDestinationsPlugin.getWorldGuardPlugin.isInRegion(npc.getEntity().getLocation(), rancherLocation.regionName)) {
                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.INFO, "DestinationsEventsListener.onNavigationNewDestination|NPC:" + npc.getId() + "|New Location, returning to destination.");
                        if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                            DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.FINEST, "Rancher_Processing.pluginTick|NPC:" + npc.getId() + "|Not at destination|"
                                    + ranchSet.destinationsTrait.currentLocation.destination + " Dist:" + npc.getEntity().getLocation().add(0.0D, -1.0D, 0.0D).distanceSquared(ranchSet.destinationsTrait.currentLocation.destination));
                        ...
                        DestinationsPlugin.Instance.getPathClass.addToQueue(npc, ranchSet.destinationsTrait, npc.getEntity().getLocation().add(0.0D, -1.0D, 0.0D), ranchSet.destinationsTrait.currentLocation.destination, 120,
                                new ArrayList<Material>(), 0, true, true, true, "DestinationsRancher");
                        ranchSet.lastAction = new Date();
                        ((NPCSetting) DestinationsRancher.Instance.npcSettings.get(npc.getId())).currentAction = NPCSetting.CurrentAction.TRAVERSING;
                        return;
                    }

                    for (EntityType workingAnimal : rancherLocation.enabledAnimals) {

                        results = DestinationsRancher.Instance.getProcessingClass.getValidActions(npc, rancherLocation, ActionType.SLAUGHTER, ranchSet.lastInteraction, workingAnimal);
                        if (results.closest != null) {
                            if (rancherLocation.enabledActions.contains(ActionType.SLAUGHTER) && results.validAnimals.size() > rancherLocation.maxAdults) {
                                if (results.distance < 5) {
                                    if (DestinationsRancher.Instance.getPluginReference.monitorDrops.containsKey(results.closest.getEntity().getUniqueId()))
                                        DestinationsRancher.Instance.getPluginReference.monitorDrops.remove(results.closest.getEntity().getUniqueId());

                                    if (DestinationsRancher.Instance.getDestinationsPlugin.debugTargets != null) {
                                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.sendDebugMessage("rancher", "Debug_Messages.killing_animal", npc, npcTrait);
                                    }
                                    ...
                                    DestinationsRancher.Instance.getPluginReference.monitorDrops.put(results.closest.getEntity().getUniqueId(), monEnt);
                                    ...
                                    if (DestinationsRancher.Instance.getDestinationsPlugin.debugTargets != null) {
                                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.sendDebugMessage("rancher", "Debug_Messages.walking_animal", npc, npcTrait);
                                    }
                                    continue;
                                }
                            }
                        ...
                        results = DestinationsRancher.Instance.getProcessingClass.getValidActions(npc, rancherLocation, ActionType.BREED, ranchSet.lastInteraction, workingAnimal);
                        if (results.closest != null) {
                            if (DestinationsRancher.Instance.getProcessingClass.getBabyCount(npc, rancherLocation, workingAnimal) < rancherLocation.maxBabies && rancherLocation.enabledActions.contains(ActionType.BREED)
                                    ...
                                    for (Player plr : Bukkit.getServer().getOnlinePlayers()) {
                                        if (plr.getLocation().getWorld().equals(results.closest.getEntity().getWorld())) {
                                            if (plr.getLocation().distanceSquared(results.closest.getEntity().getLocation()) < 30 * 30) {
                                                DestinationsRancher.Instance.getDestinationsPlugin.getParticleManager.PlayOutHeartParticle(results.closest.getEntity().getLocation().clone().add(0, 1, 0), plr);
                                                DestinationsRancher.Instance.getDestinationsPlugin.getParticleManager.PlayOutHeartParticle(results.closest.getEntity().getLocation().clone().add(0.2, 0.5, -0.2), plr);
                                            }
                                        }
                                    }

                                    DestinationsRancher.Instance.breedingHistory.put(results.closest.getEntity().getUniqueId(), new Date());
                                    continue;
                                } else if (walkToLocation(npc, results.closest.getEntity().getLocation(), ranchSet)) {
                                    if (DestinationsRancher.Instance.getDestinationsPlugin.debugTargets != null) {
                                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.sendDebugMessage("rancher", "Debug_Messages.walking_animal", npc, npcTrait);
                        ...
                        results = DestinationsRancher.Instance.getProcessingClass.getValidActions(npc, rancherLocation, ActionType.MILK, ranchSet.lastInteraction, workingAnimal);
                        if (results.closest != null) {
                            if (rancherLocation.enabledActions.contains(ActionType.MILK) && results.validAnimals.size() > 0) {
                                if (results.distance < 5) {
                                    ...
                                    if (DestinationsRancher.Instance.milkingHistory.containsKey(results.closest.getEntity().getUniqueId()))
                                        DestinationsRancher.Instance.milkingHistory.remove(results.closest.getEntity().getUniqueId());

                                    DestinationsRancher.Instance.milkingHistory.put(results.closest.getEntity().getUniqueId(), new Date());
                                    ranchSet.lastInteraction = results.closest.getEntity().getUniqueId();

                                    ItemStack milk = new ItemStack(Material.MILK_BUCKET, (int) 1);
                                    DestinationsRancher.Instance.addToInventory(npc, new ItemStack[] { milk });
                                    DestinationsRancher.Instance.milkingHistory.put(results.closest.getEntity().getUniqueId(), new Date());
                                    continue;
                                } else if (walkToLocation(npc, results.closest.getEntity().getLocation(), ranchSet)) {
                                    if (DestinationsRancher.Instance.getDestinationsPlugin.debugTargets != null) {
                        ...
                        results = DestinationsRancher.Instance.getProcessingClass.getValidActions(npc, rancherLocation, ActionType.SHEAR, ranchSet.lastInteraction, workingAnimal);
                        if (results.closest != null) {
                            if (rancherLocation.enabledActions.contains(ActionType.SHEAR) && results.validAnimals.size() > 0) {
                                if (results.distance < 5) {
                                    ...
                                    if (rancherLocation.enabledActions.contains(ActionType.STORE_ITEMS)) {
                                        ItemStack wool = new ItemStack(DestinationsRancher.Instance.getAnimalHelper.getWool(), (int) (Math.random() * 3) + 1);
                                        wool.setData(new Wool(results.closest.getColor()));
                                        DestinationsRancher.Instance.addToInventory(npc, new ItemStack[] { wool });
                                    }
    ...
    public ValidActionResults getValidActions(NPC npc, LocationSetting locSetting, ActionType action, UUID lastInteraction, EntityType targetAnimals) {
        ...
        for (RanchAnimal animal : this.regionGetAnimals(npc.getEntity().getLocation().getWorld(), locSetting.regionName, targetAnimals)) {
            ...
            switch (action) {
            case SLAUGHTER:
                if (animal.getHealth() > 0 && !DestinationsRancher.Instance.breedingHistory.containsKey(animal.getEntity().getUniqueId())) {
                ...
                break;
            case MILK:
                if ((animal.getEntity().getType() == EntityType.COW && DestinationsRancher.Instance.itemInHand(locSetting, npc, Material.BUCKET)) || (animal.getEntity().getType() == EntityType.MUSHROOM_COW && DestinationsRancher.Instance
                        .itemInHand(locSetting, npc, Material.BOWL))) {
                    if (!DestinationsRancher.Instance.milkingHistory.containsKey(animal.getEntity().getUniqueId())) {
                ...
                break;
            case SHEAR:
                if (animal.getEntity().getType() == EntityType.SHEEP && DestinationsRancher.Instance.itemInHand(locSetting, npc, Material.SHEARS)) {
                    if (!animal.isSheered()) {
                        validResults.validAnimals.add(animal);
            ...
            case BREED:
                ...
                if (DestinationsRancher.Instance.breedingHistory.containsKey(animal.getEntity().getUniqueId()))
                    continue;

                if (animal.canBreed() && !DestinationsRancher.Instance.breedingHistory.containsKey(animal.getEntity().getUniqueId())) {
    ...
    private boolean walkToLocation(NPC npc, Location newLocation, NPCSetting ranchSet) {
        ...
        ranchSet.lastAction = new Date();
        if (DestinationsRancher.Instance.getDestinationsPlugin != null)
            DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.FINEST, "Rancher_Processing.pluginTick|NPC:" + npc.getId() + "|NewLocation|" + newLocation + "Walkto: " + walkToLocation + " Dist:" + npc
                    .getEntity().getLocation().add(0.0D, -1.0D, 0.0D).distanceSquared(walkToLocation) + " Block:" + newLocation.getBlock().getType().toString());
        ...
        if (npc.getEntity().getLocation().distanceSquared(walkToLocation) < 300)
            ...
            DestinationsPlugin.Instance.getPathClass.addToQueue(npc, ranchSet.destinationsTrait, npc.getEntity().getLocation().add(0.0D, -1.0D, 0.0D), walkToLocation, 120, new ArrayList<Material>(), 0, true, true, true, "DestinationsRancher");
                        ...
                        for (Entity ent : checkLocation.getChunk().getEntities()) {
                            ...
                            if (allowedAnimal.equals(ent.getType())) {
                                if (ent.getLocation().getBlockY() >= regionBounds[0].getBlockY() && ent.getLocation().getBlockY() <= regionBounds[1].getBlockY()) {
                                    ...
                                    {
                                        foundAnimals.add(DestinationsRancher.Instance.getAnimalHelper.getRanchAnimal(ent));
                                    }
                                }
                            }
