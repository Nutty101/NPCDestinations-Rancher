...
import net.livecar.nuttyworks.destinations_rancher.plugin.RancherAddon;
import net.livecar.nuttyworks.destinations_rancher.plugin.RancherCommands;
import net.livecar.nuttyworks.destinations_rancher.plugin.RancherProcessing;
...
public class DestinationsRancher {
    // For quick reference to this instance of the plugin.
    public static DestinationsRancher Instance = null;

    // Links to classes
    public Citizens getCitizensPlugin;
    public DestinationsPlugin getDestinationsPlugin = null;
    public DestRancherPlugin getPluginReference = null;
    public RancherProcessing getProcessingClass = null;
    public RancherAddon getRancherPlugin = null;
    ...
    public List<EntityType> supportedAnimals = new ArrayList<EntityType>();
    ...
    public DestinationsRancher() {
        this.getRancherPlugin = new RancherAddon(this);
        DestinationsPlugin.Instance.getPluginManager.registerPlugin(getRancherPlugin);
        DestinationsPlugin.Instance.getCommandManager.registerCommandClass(RancherCommands.class);
        this.getProcessingClass = new RancherProcessing();
    }
            ...
            for (int slot = 1; slot < npcInventory.length; slot++) {
                ...
                if (npcInventory[slot] != null)
                    if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.INFO, "RancherProcessing.addToInventory|NPC:" + npc.getId() + "|Slot:" + slot + " Item:" + npcInventory[slot].getType() + "/"
                                + npcInventory[slot].getAmount() + "/" + npcInventory[slot].getType().getMaxStackSize() + " Inv Item:" + item.getType() + "/" + item.getAmount());
                if (npcInventory[slot] == null)
                    if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                        DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.INFO, "RancherProcessing.addToInventory|NPC:" + npc.getId() + "|Slot: null" + " Inv Item:" + item.getType() + "/" + item
                                .getAmount());

                if (npcInventory[slot] != null) {
                    if (npcInventory[slot].getType() == item.getType() && npcInventory[slot].getAmount() < npcInventory[slot].getType().getMaxStackSize()) {
                        if (DestinationsRancher.Instance.getDestinationsPlugin != null)
                            DestinationsRancher.Instance.getDestinationsPlugin.getMessageManager.debugMessage(Level.INFO, "RancherProcessing.addToInventory|NPC:" + npc.getId() + "|SlotCheck: " + npcInventory[slot].getAmount() + item
